  ...
  module Actions
    ...
    class SmmcS3Action < Action
      def self.run(config)
        ...
        params[:title_suffix] = config[:title_suffix]
      ...
      def self.upload_ipa(s3_client, params, s3_region, s3_access_key, s3_secret_access_key, s3_bucket, ipa_file, dsym_file, s3_path, acl)
        ...
        if plist_template_path && File.exist?(plist_template_path)
          ...
          puts "2 - #{Helper.gem_path('fastlane-plugin-smmc_s3')}"
      ...
      def self.available_options
        [
         ...
         FastlaneCore::ConfigItem.new(key: :region,
          ...
          FastlaneCore::ConfigItem.new(key: :title_suffix,
                                       env_name: "",
                                       description: "PLIST-name-suffix",
                                       optional: true,
                                       default_value: "public-read"
                                      )
        ]
